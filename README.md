...
question files. The current Aux11 curated file is passed privately to the
selector. The final Prompt + postprocess teacher level is used only for coarse
sampling strata; raw `difficulty` and `raw_difficulty` are never read.
...
The CPU-only selector never loads the backbone model and does not require old
BT scores. It first reserves up to 1,000 questions for each final teacher level,
allocates the remaining capacity proportionally to the source level
distribution, enforces Aux11 category floors globally and per level, and then
uses an 80/10/10 mix of marginal matching, rare-category protection, and
deterministic exploration. Old V3 training nodes and every validation/test node
are explicitly excluded. A private audit file retains levels and features; the
teacher-facing question file contains neither.

The pair builder protects every observed Aux11 category, limits the maximum
