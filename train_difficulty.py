...
def save_checkpoint(model: torch.nn.Module, tokenizer: Any, optimizer: torch.optim.Optimizer, scheduler: Any, output_dir: Path, epoch: int, optimizer_step: int, args: argparse.Namespace) -> Path:
    ...
    (checkpoint / "trainer_state.json").write_text(json.dumps({"next_epoch": epoch, "optimizer_step": optimizer_step, "seed": args.seed}, ensure_ascii=False, indent=2), encoding="utf-8")
    return checkpoint
...
def main() -> None:
    ...
    if resume_dir:
        ...
        start_epoch, optimizer_step = int(state["next_epoch"]), int(state["optimizer_step"])
    ...
    if main_process:
        ...
        print(json.dumps({"message": "Training reports loss only; run evaluate_difficulty.py separately for model selection.", "start_epoch": start_epoch, "total_epochs": args.num_train_epochs}, ensure_ascii=False), flush=True)
    ...
    for epoch in range(start_epoch, args.num_train_epochs):
        ...
        total_loss, accumulated_micro_steps = 0.0, 0
        ...
        if main_process:
            ...
            if (epoch + 1) % args.checkpoint_every_epochs == 0 or epoch + 1 == args.num_train_epochs:
                checkpoint = save_checkpoint(model, tokenizer, optimizer, scheduler, output_dir, epoch + 1, optimizer_step, args)
