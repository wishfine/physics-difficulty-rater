...
十个辅助头全部是独立的**单标签分类头**。对一道题，每个头只能从自己的候选类别中选择
一个类别，使用 softmax 和 CrossEntropy 训练。

```yaml
head_type: independent_Linear
task_type: single_label_classification
activation_for_prediction: softmax
total_heads: 10
total_auxiliary_logits: 46
multi_label_heads: 0
```
...
#### 1. `problem_structure`：题目的主要任务结构

这个头判断学生解题时面对的**核心任务类型**，不是简单统计题面出现了哪些关键词。

| 候选类别 | 人话解释 |
|---|---|
| 概念判断 | 主要考查概念辨析、现象判断、规律理解，通常不需要完整计算链 |
| 直接计算 | 已知量和目标量明确，主要通过公式代入或短计算得到答案 |
| 实验探究 | 核心任务是实验操作、控制变量、故障分析、方案设计或误差评价 |
| 图像表格分析 | 核心信息来自图像、图线或表格，需要读数、比较、归纳、反推或外推 |
| 电路综合 | 以电路连接、动态电路、故障、电功率或多电学关系综合分析为主 |
| 力学综合 | 综合使用受力、运动、压强、浮力、功和机械能等力学关系 |
| 热学综合 | 综合使用物态变化、热量、比热容、内能或热效率等关系 |
| 光学声学综合 | 以光学或声学过程及其综合分析为主 |
| 跨模块综合 | 必须联合两个及以上物理模块才能完成主要推理 |

这里判断的是“主结构”。例如，电路题中附带一张读数图，如果主要难点仍是动态电路关系，
应标为“电路综合”，而不是因为出现图像就必然标为“图像表格分析”。

#### 2. `step_count`：必要物理推理步骤数

这个头估计从题目条件到答案所需的**有效物理推理步骤**，不是按解析句号、公式行数或
机械计算次数计数。
...
这个头判断的是计算组织难度，而不是数字看起来大不大。

| 候选类别 | 人话解释 |
|---|---|
| 口算或直接判断 | 无需正式笔算，或只需非常直接的数量判断 |
| 简单笔算 | 单公式或少量常规运算即可完成 |
| 多公式联立 | 需要多个公式连续代入、联立或分阶段计算 |
| 复杂方程或范围计算 | 涉及复杂方程、分情况、区间、极值、临界值或允许范围 |

连续进行很多次相同的简单运算，不一定等于“复杂方程或范围计算”。

#### 4. `reasoning_chain`：主要推理方式
...
|---|---|
| 直接套用 | 识别公式或规律后直接代入 |
| 简单因果推理 | 需要一层或少量前因后果判断 |
| 多层因果推理 | 中间结论继续作为后续条件，形成多层连续推导 |
| 逆向推理或临界分析 | 需要从结果反推条件，或分析边界、极值、恰好发生等临界情况 |

#### 5. `knowledge_count`：连续推理中使用的知识点数量

这个头统计真正参与同一解题链的知识点，而不是把所有选项中互不相关的知识点机械相加。

| 候选类别 | 人话解释 |
|---|---|
| 1个 | 主要依赖一个核心知识点 |
| 2-3个 | 两到三个知识点需要衔接使用 |
| 4个及以上 | 至少四个知识点共同参与连续推理 |

例如四个独立判断选项各考一个概念，不应自动算作“4个及以上”；只有这些知识点需要在同一
推理过程中联合使用时才计入。

#### 6. `subquestion_dependency`：多小问之间的依赖关系

这个头区分“问题数量多”和“前后真正递进”。

| 候选类别 | 人话解释 |
|---|---|
| 无多问 | 题目没有多个需要分别作答的小问 |
| 多问但相互独立 | 有多个小问，但各问可以直接从题目原始条件独立完成 |
| 多问且层层递进 | 后一问依赖前一问的结论、变量或中间结果 |

#### 7. `state_count`：需要比较或追踪的物理状态数

这个头描述题目包含多少个需要建模的状态，以及状态是否连续变化。

| 候选类别 | 人话解释 |
|---|---|
| 单状态 | 只分析一个稳定状态或一个时刻 |
| 双状态 | 需要比较前后、开关变化前后或两个明确状态 |
| 多状态 | 至少三个离散状态需要分别分析和关联 |
| 连续变化或临界状态 | 状态连续变化，或需要找到变化过程中的临界点、极值、恰好条件 |

#### 8. `constraint_count`：有效边界或约束条件数量

这个头判断安全范围、边界、守恒限制、器材量程、最大最小值等约束的复杂度。

| 候选类别 | 人话解释 |
|---|---|
| 无约束 | 没有额外边界条件，按常规关系直接求解 |
| 单一约束 | 只需同时满足一个主要限制条件 |
| 多约束 | 多个限制必须联合满足，通常涉及取交集、排除或综合边界 |

题目中普通已知量不自动算作约束；只有会限制可行解或取值范围的条件才计入。

#### 9. `variable_relation`：变量之间的建模关系

这个头判断是否需要理解或建立变量之间的函数、比例或耦合关系。

| 候选类别 | 人话解释 |
|---|---|
| 无变量关系 | 只求固定值或做定性判断，不需要研究变量如何联动 |
| 简单正反比 | 主要分析一组简单正比、反比或单调关系 |
| 图像函数关系 | 需要从图线、斜率、截距、面积或函数形状理解变量关系 |
| 多变量耦合关系 | 多个变量相互制约，一个量变化会通过多个关系影响其他量 |

#### 10. `information_processing`：图表和实验信息加工要求

这个头由旧 frozen18 的 `graph_table_requirement` 和 `experiment_requirement` 合并而来，
判断学生如何处理非纯文字条件。

| 候选类别 | 人话解释 |
|---|---|
| 无 | 不需要额外图表或实验信息加工 |
| 图表直接读数 | 从图或表中直接读取一个或少量数值 |
| 图表多组比较归纳 | 比较多组图表数据并总结规律 |
| 图像反推或外推 | 根据图线反推条件、求未知量或预测区间外趋势 |
| 实验基础操作或读数 | 识别器材、操作步骤、量程、分度值或完成基础读数 |
| 实验控制变量或故障分析 | 分析控制变量、实验条件或装置/电路故障 |
| 实验方案设计或误差评价 | 设计实验、改进方案、分析误差来源或评价方案合理性 |
| 图表与实验混合处理 | 同时需要实验推理和图表信息加工 |

如果旧数据中的图表要求和实验要求都不为“无”，转换后统一标为“图表与实验混合处理”。

### 7.2 十个头的输出如何使用

对每个头，模型输出该头所有类别的 logits，推理时取 softmax 概率最大的类别：
...
└── information_processing: Linear(H, 8)
```

这些输出有三个用途：

1. 作为训练时的辅助任务，促使 backbone 学习与题目难度有关的过程结构；
2. 在推理时提供可解释标签；
3. 做数据覆盖、类别偏置和错误切片分析。
...
### 8.1 相较原绝对五档模型做了哪些结构调整

原来的绝对分类模型 `QwenDifficultyRater` 直接预测五档：
...
QuRating pairwise 模型 `QwenPairwiseRater` 将它改成：

```text
Question A → shared scorer → s(A)
Question B → shared scorer → s(B)
                         ↓
                 pair logit = s(A) - s(B)
                         ↓
                sigmoid → P(A harder than B)
```

具体变化如下：

| 部分 | 原绝对五档模型 | QuRating V3 模型 |
|---|---|---|
...
| 推理输出 | 五档概率 | 单题 raw scalar；经冻结 calibration 后得到五档 |
| 辅助头 | 固定存在十个头 | V1 不加载；V2/V3 可选加载 |
| 外部规则 | 曾考虑特征升降档 | 明确取消，辅助头只解释不改难度 |

这次结构调整的核心不是“在五分类头外再加一个 pair 分类头”，而是**彻底移除五档主头，
把主任务改成共享单题标量**。pair 概率由两个标量做差自然得到，因此不存在一个依赖 A/B
顺序的独立二分类器。
...
模型没有增加额外 MLP。标量头和十个辅助头都直接连接共享 LayerNorm 后的表示。训练代码
还隔离了辅助头初始化的随机数状态，使相同 seed 下 V1 和 V2/V3 的主干初始化及 Dropout
随机流保持可比，减少对照实验中的非目标差异。
